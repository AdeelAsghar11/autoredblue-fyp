...
4. **Deterministic classical planner (PEP / CHECKMATE) + Task Difficulty Assessment.** Externalise long-horizon attack-path planning to a real classical planner with a formal DAG of the attack surface and TDA-based pruning, letting a small edge model rival larger cloud models on multi-step engagements. *(Finding 4, the biggest and most promising future extension. Cite CHECKMATE, arXiv:2512.11143, ~20% higher success / ~50% cheaper, already used commercially by the tool "numasec"; and PentestGPT V2, arXiv:2602.17622, titled "Excalibur" in the original research doc, for the Task Difficulty Assessment mechanism.)*
